            ...
            <div class="card-body p-4">
                <form method="POST" action="{{ route('admin.users.update', $user->id) }}">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6 mb-3" id="shift-container" style="display: none;">
                            ...
                            <select name="shift" class="form-select @error('shift') is-invalid @enderror">
                                ...
                                <option value="morning" {{ old('shift', $user->shift) == 'morning' ? 'selected' : '' }}>Morning Shift</option>
                                <option value="day" {{ old('shift', $user->shift) == 'day' ? 'selected' : '' }}>Day Shift</option>
                                <option value="night" {{ old('shift', $user->shift) == 'night' ? 'selected' : '' }}>Night Shift</option>
                            </select>
                        ...
                        <div class="col-md-6 mb-3" id="area-container" style="display: none;">
                            ...
                            <input type="text" name="area" class="form-control @error('area') is-invalid @enderror" value="{{ old('area', $user->area) }}">
                          ...
                          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:8px; max-height:200px; overflow-y:auto; border:1px solid #e2e8f0; border-radius:6px; padding:10px">
                            @foreach($cameras as $camera)
                              <label style="display:flex; align-items:center; gap:8px; font-size:13px">
                                <input type="checkbox" name="cameras[]" value="{{ $camera->id }}" {{ in_array($camera->id, old('cameras', $user->cameras->pluck('id')->toArray())) ? 'checked' : '' }}>
                                {{ $camera->name }}
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-teal px-4">Update User</button>
                    </div>
                </form>
            </div>
        </div>
